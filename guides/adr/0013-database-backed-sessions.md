...
3. **Session fixation**
    1. Delete any old sessions on login, and set a new session_id
    2. Use ungeussable session_id
...
create_table :session do |t|
  t.string :id_token
  t.string :session_id, null: false # An ungeussable value to prevent fixation attacks
  t.string :ip_address
  t.string :user_agent
  ...
  add_index :session, :session_id, unique: true, if_not_exists: true
end
...
class Find::SessionsController
  ...
  def callback
    unless authenticated?
      ...
      # Ensure any previous sessions are destroyed
      Session.destroy_by(session_id: CandidateSession.new(cookies).session_id)

      # Generate a new session id in the cookie
      session_id = SecureRandom.hex(32)
      Session.create(sessionable: candidate, session_id:)
      CandidateSession.update({session_id:))
    end
