...
2. This is the Google geocode service. It returns a matching place and the corresponding `place_id`.
3. We use this `place_id` to get the coordinates of the place  from Google.
...
We take the `place_id` from the `Geolocation::Suggestions` and use that to fetch the coordinates of the place from the geocode API in Google.
