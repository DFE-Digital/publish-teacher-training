...
RSpec.describe "Saving a course", :js, service: :find do
  ...
  def and_i_remain_on_the_course_page
    expect(page).to have_content(@course.name)
    expect(page).to have_current_path(find_course_path(provider_code: @course.provider.provider_code, course_code: @course.course_code))
  end
  ...
  def when_i_unsave_the_course
    click_on("Course saved")
  end
