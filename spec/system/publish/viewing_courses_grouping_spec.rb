...
RSpec.describe "Viewing the grouped course list" do
  ...
  def then_i_see_a_section_per_group
    expect(page).to have_css(".app-table--courses__section", count: 3)
  end
  ...
  def and_the_ratified_sections_are_in_alphabetical_order
    headings = page.all(".app-table--courses__section h2").map { |h2| h2.text.squish }
  ...
  def then_i_see_no_course_sections
    ...
    expect(page).to have_no_css(".app-table--courses__section")
  end
end
