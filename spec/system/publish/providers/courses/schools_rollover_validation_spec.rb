...
RSpec.describe "Publish - Schools validation during 2026 rollover", service: :publish, travel: 1.day.before(find_closes(2025)) do
  ...
  let!(:course)   { create(:course, :publishable, provider:, course_code: "XYZ", sites: [site_one, site_two], accrediting_provider: accredited_provider, enrichments: [build(:course_enrichment, :rolled_over, :v2)]) }
