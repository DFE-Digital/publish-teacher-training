...
RSpec.describe "Publish - Select all schools", :js, type: :system do
  ...
  def given_i_am_authenticated_as_a_provider_user
    ...
    create(:site, :with_corresponding_provider_school, provider: @provider, location_name: "Site 1")
    create(:site, :with_corresponding_provider_school, provider: @provider, location_name: "Site 2")
    create(:site, :with_corresponding_provider_school, provider: @provider, location_name: "Site 3")
    @user = create(
      :user,
      providers: [provider],
    )
  ...
  def given_there_are_many_schools
    @schools = create_list(:site, 30, :with_corresponding_provider_school, provider: @provider)
  end

  def given_the_provider_has_25_schools
    # the provider already has 3 sites from authentication setup
    create_list(:site, 22, :with_corresponding_provider_school, provider: @provider)
    @provider.reload
  end
