...
# The valid range runs from today to the apply deadline, so the valid-date
# scenario has no date to use on the last two days of the cycle. The provider
# factory follows the current cycle, so no year is needed here.
RSpec.describe "Add course wizard visa sponsorship application deadline at step", travel: mid_cycle, type: :system do
  before do
