...
feature 'choosing an application open from date', { can_edit_current_and_next_cycles: false } do
  ...
  def when_i_visit_the_new_applications_open_from_page
    publish_courses_new_applications_open_page.load(provider_code: provider.provider_code, recruitment_cycle_year: Settings.current_recruitment_cycle_year, query: applications_open_from_params)
  end
  ...
  def when_i_select_on_a_specific_date
    publish_courses_new_applications_open_page.applications_open_field_day.set('1')
    publish_courses_new_applications_open_page.applications_open_field_month.set('1')
    publish_courses_new_applications_open_page.applications_open_field_year.set(Settings.current_recruitment_cycle_year)
    publish_courses_new_applications_open_page.applications_open_field_other.click
  end

  def and_i_put_in_a_custom_date
    publish_courses_new_applications_open_page.applications_open_field_day.click
  end
