...
feature "Viewing courses as an accredited body", { can_edit_current_and_next_cycles: false } do
  ...
  def then_i_should_see_a_list_of_training_providers
    expect(training_provider_index_page.training_provider_rows.size).to eq(1)

    expect(training_provider_index_page.training_provider_rows.first.name).to have_text(training_provider.provider_name)
  end
  ...
  def and_i_click_on_a_training_provider
    training_provider_index_page.training_provider_rows.first.name.click
  end

  def then_i_see_the_courses_i_accredit_for
    expect(course_index_page).to be_displayed
    expect(course_index_page.courses.size).to eq(1)
    expect(course_index_page.courses.first.name).to have_text(course.name)
  end
