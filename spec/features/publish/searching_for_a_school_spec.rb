...
feature 'Searching for a school from the GIAS list' do
  ...
  def then_i_see_the_school_i_searched_for
    expect(page.find_by_id('support-school-form-location-name-field').value).to eq(@school.name)
  end
  ...
  def and_the_school_form_should_be_prefilled_with_the_school_details
    expect(page).to have_field('School name', with: @school_two.name)
    expect(page).to have_field('URN', with: @school_two.urn)

    expect(page).to have_field('Address line 1', with: @school_two.address1)
    expect(page).to have_field('Town or city', with: @school_two.town)
    expect(page).to have_field('Postcode', with: @school_two.postcode)
  end
