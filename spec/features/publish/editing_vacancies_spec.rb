...
feature 'Editing vacancies', { can_edit_current_and_next_cycles: false } do
  ...
  def when_i_visit_the_vacancy_edit_page_for_a_course
    publish_course_vacancies_edit_page.load(
      provider_code: provider.provider_code, recruitment_cycle_year: provider.recruitment_cycle_year, course_code: course.course_code
    )
  end

  def and_i_submit_with_invalid_data
    publish_course_vacancies_edit_page.submit.click
  end

  def then_i_should_see_a_an_error_message
    expect(publish_course_vacancies_edit_page.error_messages).to include(vacancies_confirmation_error_message)
  end

  def then_i_should_see_a_checkbox_to_turn_off_all_vacancies
    expect(publish_course_vacancies_edit_page).to have_content('I confirm there are no vacancies')
    expect(publish_course_vacancies_edit_page).to have_content('Close this course')
    expect(publish_course_vacancies_edit_page).to have_confirm_no_vacancies
    expect(publish_course_vacancies_edit_page.confirm_no_vacancies).not_to be_checked
  end

  def then_i_should_see_a_checkbox_to_turn_on_all_vacancies
    expect(publish_course_vacancies_edit_page).to have_content('I confirm there are vacancies')
    expect(publish_course_vacancies_edit_page).to have_content('Reopen this course')
    expect(publish_course_vacancies_edit_page).to have_confirm_has_vacancies
    expect(publish_course_vacancies_edit_page.confirm_has_vacancies).not_to be_checked
  end
  ...
  def then_i_should_see_the_course_as_having_no_vacancies
    expect(publish_course_vacancies_edit_page).to have_vacancies_radio_choice
    expect(publish_course_vacancies_edit_page.vacancies_radio_no_vacancies).to be_checked
    expect(publish_course_vacancies_edit_page.vacancies_radio_has_some_vacancies).not_to be_checked
    expect(publish_course_vacancies_edit_page.vacancy_checked_values).to all(be_falsey)
  end
  ...
  def then_i_should_see_a_checkbox_for_each_study_mode_per_site
    ...
    expect(publish_course_vacancies_edit_page).to have_vacancies_radio_choice
    expect(publish_course_vacancies_edit_page.vacancies_radio_has_some_vacancies).to be_checked
    expect(publish_course_vacancies_edit_page.vacancy_names).to match_array(expected_checkbox_state.keys)
    expect(publish_course_vacancies_edit_page.vacancy_checked_values).to match_array(expected_checkbox_state.values)
  end

  def when_i_check_the_checkbox_to_turn_off_all_vacancies
    publish_course_vacancies_edit_page.confirm_no_vacancies.check
  end

  def when_i_check_the_checkbox_to_turn_on_all_vacancies
    publish_course_vacancies_edit_page.confirm_has_vacancies.check
  end
  ...
  def and_i_check_all_the_sites
    ...
    publish_course_vacancies_edit_page.vacancies_radio_has_some_vacancies.choose
    publish_course_vacancies_edit_page.vacancies.each(&:check)
  end

  def and_i_uncheck_some_sites
    expect(publish_course_vacancies_edit_page.vacancies_radio_has_some_vacancies).to be_checked

    publish_course_vacancies_edit_page.vacancies.find { |el| el.text == 'Site 1' }.uncheck
  end
  ...
  def and_i_check_some_sites
    ...
    publish_course_vacancies_edit_page.vacancies.find { |el| el.text == 'Site 1' }.check
  end

  def and_i_check_the_part_time_site
    expect(publish_course_vacancies_edit_page.vacancies_radio_has_some_vacancies).to be_checked

    publish_course_vacancies_edit_page.vacancies.find { |el| el.text == 'Uni 1 (Part time)' }.check
  end
