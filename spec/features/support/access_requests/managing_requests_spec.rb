...
feature 'Viewing and approving requests', { can_edit_current_and_next_cycles: false, with_publish_constraint: true } do
  ...
  def then_i_should_see_a_link_to_view_the_requests
    expect(support_footer_page).to have_access_requests_link
    expect(support_footer_page.access_requests_link).to have_text('Access Requests (2)')
  end

  def when_i_click_on_the_link
    support_footer_page.access_requests_link.click
  end

  def then_i_see_the_requests
    expect(support_access_requests_index_page).to be_displayed
    expect(support_access_requests_index_page.requests.size).to eq(2)
  end

  def when_i_view_the_first_request
    support_access_requests_index_page.load
    support_access_requests_index_page.requests.first.view_request.click
  ...
  def then_the_request_should_be_deleted
    expect(support_access_requests_index_page).to have_text('Successfully deleted the Access Request')
    expect(support_access_requests_index_page.requests.size).to eq(1)
  end
