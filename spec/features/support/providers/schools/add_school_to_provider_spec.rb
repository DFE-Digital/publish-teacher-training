...
feature 'Adding school to provider as an admin', :with_publish_constraint, { can_edit_current_and_next_cycles: false } do
  before do
  ...
  def when_i_search_with_an_unmatched_query
    fill_in 'publish-schools-search-form-query-field', with: 'zzz'
    click_continue
  end
  ...
  def when_i_search_with_an_partial_query
    fill_in 'publish-schools-search-form-query-field', with: 'Dis'
    click_continue
  end
  ...
  def then_i_see_the_confirmation_page
    ...
    expect(page).to have_content('Unique reference number (URN)123456')
