...
feature "Edit an Allocation Uplift" do
  before do
    ...
    when_i_visit_the_support_allocation_uplift_new_page
  end
...
private
  ...
  def then_i_can_create_an_uplift
    ...
    expect(support_allocation_uplift_new_page).to have_content "Create an Allocation Uplift for #{@provider.provider_name}"
    support_allocation_uplift_new_page.allocation_uplift_amount.set(@allocation_uplift_amount)
  end

  def and_i_click_continue
    support_allocation_uplift_new_page.submit.click
  end
  ...
  def with_a_success_message
    expect(support_allocations_show_page).to have_content "Allocation Uplift was successfully created"
  end
