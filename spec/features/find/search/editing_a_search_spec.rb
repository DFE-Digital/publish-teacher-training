...
private
  ...
  def then_i_should_see_the_results_page
    ...
    expect(results_page.current_url).to end_with("/find/results?age_group=primary&has_vacancies=true&l=2&subjects%5B%5D=00")
  end
  ...
  def then_i_should_see_the_subjects_form
    expect(primary_subjects_page.current_url).to end_with(
      "/find/subjects?age_group=primary&has_vacancies=true&l=2&qualification%5B%5D=qts&qualification%5B%5D=pgce_with_qts&qualification%5B%5D=pgce+pgde&send_courses=false&study_type%5B%5D=full_time&study_type%5B%5D=part_time&subjects%5B%5D=00",
    )
  end
