...
feature 'Searching across England' do
  ...
  def and_the_correct_subjects_form_page_url_and_query_params_are_present
    URI(current_url).then do |uri|
      ...
      expect(uri.query).to eq('age_group=primary&l=2&qualification%5B%5D=qts&qualification%5B%5D=pgce_with_qts&qualification%5B%5D=pgce+pgde&send_courses=false&study_type%5B%5D=full_time&study_type%5B%5D=part_time')
    end
  end
  ...
  def then_i_should_see_the_find_results_page
    expect(page).to have_current_path('/results?age_group=primary&applications_open=true&l=2&qualification%5B%5D=qts&qualification%5B%5D=pgce_with_qts&qualification%5B%5D=pgce+pgde&send_courses=false&study_type%5B%5D=full_time&study_type%5B%5D=part_time&subjects%5B%5D=00')
  end
