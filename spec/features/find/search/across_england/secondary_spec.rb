...
private
  ...
  def then_i_should_see_a_subjects_validation_error
    expect(page).to have_content("Select at least one subject you want to teach")
  end
  ...
  def then_i_should_see_the_results_page
    expect(page).to have_current_path("/find/results?age_group=secondary&has_vacancies=true&l=2&qualification%5B%5D=qts&qualification%5B%5D=pgce_with_qts&qualification%5B%5D=pgce+pgde&send_courses=false&study_type%5B%5D=full_time&study_type%5B%5D=part_time&subjects%5B%5D=C1")
  end
