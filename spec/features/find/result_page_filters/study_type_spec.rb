...
RSpec.feature "Study type filter" do
  ...
  def and_the_full_time_study_query_parameters_are_retained
    URI(current_url).then do |uri|
      ...
      expect(uri.query).to eq("has_vacancies=true&study_type[]=full_time&qualification[]=qts&qualification[]=pgce_with_qts&qualification[]=other&degree_required=show_all_courses")
    end
  end
  ...
  def and_the_part_time_study_query_parameters_are_retained
    URI(current_url).then do |uri|
      expect(uri.path).to eq("/find/results")
      expect(uri.query).to eq("has_vacancies=true&study_type[]=part_time&qualification[]=qts&qualification[]=pgce_with_qts&qualification[]=other&degree_required=show_all_courses")
    end
  end
end
