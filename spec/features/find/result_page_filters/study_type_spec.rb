...
RSpec.feature "Study type filter" do
  ...
  def and_the_full_time_study_query_parameters_are_retained
    URI(current_url).then do |uri|
      ...
      expect(uri.query).to eq("hasvacancies=true&fulltime=true&qualifications[]=qts&qualifications[]=pgce_with_qts&qualifications[]=other&degree_required=show_all_courses")
    end
  end
  ...
  def and_the_part_time_study_query_parameters_are_retained
    URI(current_url).then do |uri|
      expect(uri.path).to eq("/find/results")
      expect(uri.query).to eq("hasvacancies=true&parttime=true&qualifications[]=qts&qualifications[]=pgce_with_qts&qualifications[]=other&degree_required=show_all_courses")
    end
  end
end
