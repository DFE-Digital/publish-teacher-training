...
RSpec.describe API::Public::V1::CoursesController do
  ...
  describe "#index" do
    ...
    context "when the recruitment cycle is after the remodel cutover year" do
      ...
      it "ignores filter[findable] when querying courses" do
        published_course = create(:course, :published, provider:)
        draft_course = create(:course, :draft_enrichment, provider:)
        ...
        expect(ids).to include(published_course.id.to_s, draft_course.id.to_s)
      end
    end
    ...
    context "when the recruitment cycle is not after the remodel cutover year" do
      ...
      it "ignores filter[findable] and does not apply Course.findable" do
        course = create(:course, provider:)
        ...
        expect(course.site_statuses).to be_empty
        expect(json_response["data"].map { |returned_course| returned_course["id"] }).to include(course.id.to_s)
      end
    end
