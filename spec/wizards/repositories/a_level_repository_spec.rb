...
RSpec.describe Repositories::ALevelRepository do
  ...
  describe "#transform_for_read" do
    ...
    it "transforms a_level_subject_requirements to subjects" do
      requirements = [{ "uuid" => "abc", "subject" => "any_subject" }]
      result = repository.transform_for_read({ a_level_subject_requirements: requirements })
      expect(result[:subjects]).to eq([{ uuid: "abc", subject: "any_subject" }])
    end
