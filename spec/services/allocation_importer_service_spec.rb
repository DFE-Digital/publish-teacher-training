...
RSpec.describe AllocationImporterService do
  ...
  let(:recruitment_cycle) { find_or_create(:recruitment_cycle, id: 2) }
    ...
    context "happy path" do
      before do
        File.open(path_to_csv, "w") do |f|
          ...
          f.write "3,#{training_provider.provider_code},#{accredited_body_provider.provider_code}\n"
        end
      end
      ...
      it "is idempotent" do
        expect { subject.execute; subject.execute }.to change(Allocation, :count).by(1)
      end
    end
