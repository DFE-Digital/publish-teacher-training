describe CourseAssignableSubjectService do
  ...
  context "secondary subjects" do
    ...
    let!(:modern_languages_parent_subject) { create(:secondary_subject, subject_name: "Modern Languages", type: :SecondarySubject).becomes(SecondarySubject) }
    let!(:biology) { create(:secondary_subject, subject_name: "Biology", type: :SecondarySubject).becomes(SecondarySubject) }
    let!(:arabic) { create(:modern_languages_subject, subject_name: "French").becomes(ModernLanguagesSubject) }
