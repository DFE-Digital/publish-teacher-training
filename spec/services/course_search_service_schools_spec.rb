...
RSpec.describe CourseSearchServiceSchools do
  describe ".call" do
    subject do
    ...
    describe "range" do
      context "when a range is specified" do
        ...
        let(:joins_scope) { class_double(Course) }
        let(:gias_school_within_scope) { double }
  ...
  describe "expand_university" do
    context "university course vs non university course" do
      ...
      let(:university_gias_school) { create(:gias_school, **over_5_miles_from_null_island) }
      let(:non_university_gias_school) { create(:gias_school, **null_island) }

      let(:university_provider) { create(:provider, provider_type: :university) }
      let(:non_university_provider) { create(:provider, provider_type: :scitt) }
      ...
      let(:university_course) do
        create(:course, provider: university_provider,
                        enrichments: [build(:course_enrichment, :published)]).tap do |course|
                          create(:course_school, course:, gias_school: university_gias_school)
                        end
      end
      ...
      let(:non_university_course) do
        create(:course, provider: non_university_provider,
                        enrichments: [build(:course_enrichment, :published)]).tap do |course|
                          create(:course_school, course:, gias_school: non_university_gias_school)
                        end
      end
  ...
  describe "filter[:radius]" do
    null_island = { latitude: 0.0, longitude: 0.0 }
    about_3_miles_north = { latitude: 0.05, longitude: 0.0 }
    about_7_miles_north = { latitude: 0.1, longitude: 0.0 }

    subject do
      described_class.call(
        filter: { **null_island, radius: 5 },
        sort: nil,
        course_scope: Course.all,
      )
    end

    let(:near_course) do
      create(:course, provider: create(:provider),
                      enrichments: [build(:course_enrichment, :published)]).tap do |course|
                        create(:course_school, course:, gias_school: create(:gias_school, **about_3_miles_north))
                      end
    end
    ...
    let(:far_course) do
      create(:course, provider: create(:provider),
                      enrichments: [build(:course_enrichment, :published)]).tap do |course|
                        create(:course_school, course:, gias_school: create(:gias_school, **about_7_miles_north))
                      end
    end
    ...
    it "returns only courses whose school is within the radius" do
      expect(subject).to include(near_course)
      expect(subject).not_to include(far_course)
    end
  end

  describe "sort by distance with multiple schools per course" do
    null_island = { latitude: 0.0, longitude: 0.0 }
    about_3_miles_north = { latitude: 0.05, longitude: 0.0 }
    about_7_miles_north = { latitude: 0.1, longitude: 0.0 }
    ...
    let(:course_with_two_schools) do
      create(:course, provider: create(:provider),
                      enrichments: [build(:course_enrichment, :published)]).tap do |course|
                        create(:course_school, course:, gias_school: create(:gias_school, **about_3_miles_north))
                        create(:course_school, course:, gias_school: create(:gias_school, **about_7_miles_north))
    ...
    before do
      course_with_two_schools
    end

    it "uses the nearest school's distance" do
      expect(subject.first).to eq(course_with_two_schools)
      expect(subject.first.distance).to be_within(0.5).of(3.46)
    end
  end
end
