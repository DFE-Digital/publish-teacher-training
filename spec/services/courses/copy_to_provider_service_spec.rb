...
RSpec.describe Courses::CopyToProviderService do
  ...
  it "resets first_published_datetime on the rolled-over course" do
    course.update_column(:first_published_datetime, Time.zone.now)

    service.execute(course:, new_provider:)

    expect(new_course.first_published_datetime).to be_nil
  end
