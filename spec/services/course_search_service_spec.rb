  ...
  describe ".call" do
    ...
    before do
      allow(course_scope).to receive(:includes)
        ...
        ).and_return(includes_scope)
    end

    describe "when no scope is passed" do
      subject { described_class.call(filter:) }

      let(:course_scope) { Course }
      let(:expected_scope) { includes_scope }
      let(:filter) { {} }
    ...
    describe "sort by" do
      context "ascending provider name and course name" do
        ...
        it "orders in ascending order" do
          expect(includes_scope).to receive(:ascending_canonical_order).and_return(select_scope)
      ...
      context "descending provider name and course name" do
        ...
        it "orders in descending order" do
          expect(includes_scope).to receive(:descending_canonical_order).and_return(select_scope)
          expect(select_scope).to receive(:select).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
        ...
        describe "expand university" do
          context "when false" do
            ...
            it "orders in descending order by distance" do
              expect(includes_scope).to receive(:joins).and_return(joins_provider_scope)
      ...
      context "unspecified" do
        ...
        it "does not order" do
          expect(includes_scope).not_to receive(:order)
          expect(subject).to eq(expected_scope)
        end
      end
    end
    ...
    describe "range" do
      context "when a range is specified" do
        ...
        it "adds the within scope" do
          expect(includes_scope).to receive(:within).with(radius, origin: [latitude, longitude]).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "when a range is not specified" do
        ...
        it "does not add the within scope" do
          expect(includes_scope).not_to receive(:within)
        end
      end
    end
    ...
    describe "filter[provider.provider_name]" do
      context "when provider name is present" do
        ...
        let(:accredited_body_scope) { class_double(Course) }
        let(:order_scope) { class_double(Course) }

        it "adds some scope" do
          expect(includes_scope).to receive(:with_provider_name).and_return(accredited_body_scope)
    ...
    describe "filter[updated_since]" do
      ...
      it "adds the changed_since scope" do
        expect(includes_scope).to receive(:changed_since).and_return(expected_scope)
        expect(subject).to eq(expected_scope)
      end
    end
    ...
    describe "filter[funding]" do
      context "when value is salary" do
        ...
        it "adds the with_salary scope" do
          expect(includes_scope).to receive(:with_salary).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "when value is all" do
        ...
        it "doesn't add the with_salary scope" do
          expect(includes_scope).not_to receive(:with_salary)
          expect(subject).to eq(expected_scope)
        end
      end
    end
    ...
    describe "filter[qualification]" do
      ...
      context "when no qualifications passed" do
        ...
        it "adds the with_qualifications scope" do
          expect(includes_scope).not_to receive(:with_qualifications)
          expect(subject).to eq(expected_scope)
        end
      end
    end
    ...
    describe "filter[with_vacancies]" do
      context "when true" do
        ...
        it "adds the with_vacancies scope" do
          expect(includes_scope).to receive(:with_vacancies).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "when false" do
        ...
        it "doesn't add the with_vacancies scope" do
          expect(expected_scope).not_to receive(:with_vacancies)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "when absent" do
        ...
        it "doesn't add the with_vacancies scope" do
          expect(includes_scope).not_to receive(:with_vacancies)
          expect(subject).to eq(expected_scope)
        end
      end
    end
    ...
    describe "filter[findable]" do
      context "when true" do
        ...
        it "adds the findable scope" do
          expect(includes_scope).to receive(:findable).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "when false" do
        ...
        it "doesn't add the findable scope" do
          expect(includes_scope).not_to receive(:findable)
          expect(subject).to eq(expected_scope)
        end
      end
    ...
    describe "filter[study_type]" do
      context "when full_time" do
        ...
        it "adds the with_study_modes scope" do
          expect(includes_scope).to receive(:with_study_modes).with(%w[full_time]).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "when part_time" do
        ...
        it "adds the with_study_modes scope" do
          expect(includes_scope).to receive(:with_study_modes).with(%w[part_time]).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "when both" do
        ...
        it "adds the with_study_modes scope with an array of both arguments" do
          expect(includes_scope).to receive(:with_study_modes).with(%w[part_time full_time]).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "when absent" do
        ...
        it "doesn't add the scope" do
          expect(includes_scope).not_to receive(:with_study_modes)
          expect(subject).to eq(expected_scope)
        end
      end
    end
    ...
    describe "filter[funding_type]" do
      context "when fee" do
        ...
        it "adds the with_funding_types scope" do
          expect(includes_scope).to receive(:with_funding_types).with(%w[fee]).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "when salary" do
        ...
        it "adds the with_funding_types scope" do
          expect(includes_scope).to receive(:with_funding_types).with(%w[salary]).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "when apprenticeship" do
        ...
        it "adds the with_funding_types scope" do
          expect(includes_scope).to receive(:with_funding_types).with(%w[apprenticeship]).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "when all" do
        ...
        it "adds the with_funding_types scope" do
          expect(includes_scope).to receive(:with_funding_types).with(%w[fee salary apprenticeship]).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "when absent" do
        ...
        it "doesn't add the scope" do
          expect(includes_scope).not_to receive(:with_funding_types)
          expect(subject).to eq(expected_scope)
        end
      end
    end
    ...
    describe "filter[degree_grade]" do
      context "when two_two" do
        ...
        it "adds the with_degree_grades scope" do
          expect(includes_scope).to receive(:with_degree_grades).with(%w[two_two]).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "when third_class" do
        ...
        it "adds the with_degree_grades scope" do
          expect(includes_scope).to receive(:with_degree_grades).with(%w[third_class]).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "when not_required" do
        ...
        it "adds the with_degree_grades scope" do
          expect(includes_scope).to receive(:with_degree_grades).with(%w[not_required]).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "when all" do
        ...
        it "adds the with_degree_grades scope" do
          expect(includes_scope).to receive(:with_degree_grades).with(%w[two_one two_two third_class not_required]).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "when absent" do
        ...
        it "doesn't add the scope" do
          expect(includes_scope).not_to receive(:with_degree_grades)
          expect(subject).to eq(expected_scope)
        end
      end
    ...
    describe "filter[subjects]" do
      context "a single subject code" do
        ...
        it "adds the subject scope" do
          expect(includes_scope).to receive(:with_subjects).with(%w[A1]).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "multiple subject codes" do
        ...
        it "adds the subject scope" do
          expect(includes_scope).to receive(:with_subjects).with(%w[A1 B2]).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "when absent" do
        ...
        it "doesn't add the scope" do
          expect(includes_scope).not_to receive(:with_subjects)
          expect(subject).to eq(expected_scope)
        end
      end
    ...
    describe "filter[send_courses]" do
      context "when true" do
        ...
        it "adds the with_send scope" do
          expect(includes_scope).to receive(:with_send).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "when false" do
        ...
        it "adds the with_send scope" do
          expect(includes_scope).not_to receive(:with_send)
          expect(subject).to eq(expected_scope)
        end
      end
    ...
    describe "multiple filters" do
      ...
      let(:salary_scope) { class_double(Course) }

      it "combines scopes" do
        expect(includes_scope).to receive(:with_salary).and_return(salary_scope)
    ...
    describe "filter[can_sponsor_visa]" do
      context "when true" do
        ...
        it "adds the provider_can_sponsor_visa scope" do
          expect(includes_scope).to receive(:provider_can_sponsor_visa).and_return(expected_scope)
          expect(subject).to eq(expected_scope)
        end
      end
      ...
      context "when false" do
        ...
        it "adds the provider_can_sponsor_visa scope" do
          expect(includes_scope).not_to receive(:provider_can_sponsor_visa)
          expect(subject).to eq(expected_scope)
        end
      end
