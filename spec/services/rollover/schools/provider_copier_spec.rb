...
RSpec.describe Rollover::Schools::ProviderCopier do
  ...
  it "copies provider-school relationships with their UUID, GIAS school and site code" do
    expect { copy_schools }.to change(new_provider.schools, :count).from(0).to(2)

    expect(new_provider.schools.pluck(:uuid, :gias_school_id, :site_code)).to contain_exactly(
      [provider_school.uuid, provider_school.gias_school_id, "B"],
      [main_site.uuid, main_site.gias_school_id, Provider::School::MAIN_SITE_CODE],
    )
  end
