...
describe "#enrichments" do
  ...
  describe "#publish_enrichments" do
    ...
    context "on a course with only a draft enrichment" do
      ...
      its(:changed_at) { is_expected.to be_within(1.second).of Time.zone.now }
      ...
      it "updates enrichment updated_at to the current time" do
        expect(enrichment.updated_at).to be_within(1.second).of Time.zone.now
      end

      it "updates last_published to the current time" do
        expect(enrichment.last_published_timestamp_utc).to be_within(1.second).of Time.zone.now
      end
