...
describe CourseEnrichment do
  ...
  describe "#publish" do
    ...
    context "when the enrichment is an initial draft" do
      ...
      its(:updated_at) { is_expected.to be_within(1.second).of Time.zone.now.utc }
      its(:last_published_timestamp_utc) { is_expected.to be_within(1.second).of Time.zone.now.utc }
