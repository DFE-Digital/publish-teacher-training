...
describe "API", :with_publish_constraint do
  path "/recruitment_cycles/{year}/providers/{provider_code}/courses" do
    get "Returns the courses for the specified provider." do
      ...
      parameter name: :sort,
        in: :query,
        schema: { enum: ["provider.provider_name,name", "-provider.provider_name,name", "name,provider.provider_name", "name,provider.provider_name"], "$ref" => "#/components/schemas/Sort" },
        type: :object,
        style: :form,
        explode: false,
        required: false,
