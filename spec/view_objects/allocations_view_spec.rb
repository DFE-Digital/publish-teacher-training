...
describe AllocationsView do
  ...
  describe "#allocation_renewals" do
    subject { AllocationsView.new(training_providers:, allocations:).repeat_allocation_statuses }

    context "Accrediting provider has re-requested an allocation for a training provider" do
      let(:repeat_allocation) do
        build(:allocation, :repeat, accredited_body:, provider: training_provider, number_of_places: 1)
      end
      let(:initial_allocation) do
        build(:allocation, :initial, accredited_body:, provider: another_training_provider, number_of_places: 3)
      end
    ...
    context "Accredited body has declined an allocation for a training provider" do
      let(:declined_allocation) { build(:allocation, :declined, accredited_body:, provider: training_provider, number_of_places: 0) }
      let(:initial_allocation) do
        build(:allocation, :initial, accredited_body:, provider: another_training_provider, number_of_places: 3)
      end
  ...
  describe "#initial_allocations" do
    subject { AllocationsView.new(training_providers:, allocations:).initial_allocation_statuses }
    ...
    context "Accredited body has requested an initial allocation for a training provider" do
      context "more than 1 place requested" do
        let(:initial_allocation) do
          build(:allocation, :initial, accredited_body:, provider: training_provider, number_of_places: 3)
        end

        let(:repeat_allocation) do
          build(:allocation, :repeat, accredited_body:, provider: another_training_provider, number_of_places: 4)
        end
      ...
      context "1 place requested" do
        let(:initial_allocation) do
          build(:allocation, :initial, accredited_body:, provider: training_provider, number_of_places: 1)
        end
    ...
    context "Accredited body has not requested initial allocations for any training provider" do
      let(:repeat_allocation) do
        build(:allocation, :repeat, accredited_body:, provider: training_provider, number_of_places: 4)
      end
  ...
  context "allocations are confirmed" do
    describe "#confirmed_allocation_places" do
      subject { AllocationsView.new(training_providers:, allocations:).confirmed_allocation_places }

      context "returns confirmed repeat and initial allocations with number of places" do
        let(:confirmed_repeat_allocation) do
          build(:allocation, :repeat, :with_allocation_uplift, accredited_body:,
        ...
        let(:confirmed_initial_allocation) do
          build(:allocation, :initial, :with_allocation_uplift, accredited_body:,
      ...
      context "no confirmed declined allocations are returned" do
        let(:confirmed_declined_allocation) do
          build(:allocation, :declined, accredited_body:, provider: training_provider, number_of_places: 0)
        end
  ...
  context "allocation period is closed" do
    describe "#requested_allocations" do
      subject { AllocationsView.new(training_providers:, allocations:).requested_allocations_statuses }
      ...
      describe "allocation ordering" do
        ...
        let(:repeat_allocation) do
          ...
          build(:allocation, :repeat, accredited_body:, provider: another_training_provider, number_of_places: 0)
        end
    ...
    describe "#not_requested_allocations" do
      subject { AllocationsView.new(training_providers:, allocations:).not_requested_allocations_statuses }
