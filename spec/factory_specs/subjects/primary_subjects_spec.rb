...
describe "PrimarySubject factory" do
  subject { find_or_create(:primary_subject) }
  ...
  context "primary_with_english" do
    subject { find_or_create(:primary_subject, :primary_with_english) }
  ...
  context "primary_with_geography_and_history" do
    subject { find_or_create(:primary_subject, :primary_with_geography_and_history) }
  ...
  context "primary_with_mathematics" do
    subject { find_or_create(:primary_subject, :primary_with_mathematics) }
    its(:subject_name) { should eq("Primary with mathematics") }
    its(:subject_code) { should eq("03") }
  end

  context "primary_with_modern_languages" do
    subject { find_or_create(:primary_subject, :primary_with_modern_languages) }
  ...
  context "primary_with_physical_education" do
    subject { find_or_create(:primary_subject, :primary_with_physical_education) }
    its(:subject_name) { should eq("Primary with physical education") }
    its(:subject_code) { should eq("06") }
  end

  context "primary_with_science" do
    subject { find_or_create(:primary_subject, :primary_with_science) }
