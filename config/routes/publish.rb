    ...
    resources :recruitment_cycles, param: :year, constraints: { year: /#{Settings.current_recruitment_cycle_year}|#{Settings.current_recruitment_cycle_year + 1}/ }, path: "", only: [:show] do
      ...
      resources :courses, param: :code, only: %i[index new create show] do
        ...
        constraints ->(_req) { FeatureFlag.active?(:long_form_content) } do
          get "/course-content/fees-and-financial-support", on: :member, to: "courses/course_content/fees_and_financial_support#edit"
          patch "/course-content/fees-and-financial-support", on: :member, to: "courses/course_content/fees_and_financial_support#update"
        end
