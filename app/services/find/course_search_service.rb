module Find
  ...
  private
    ...
    def degree_grades
      return [] if filter[:degree_required].blank? || filter[:degree_required] == "show_all_courses"

      filter[:degree_required].split(",")
    end

    def subject_codes
      return [] if filter[:subject_codes].blank?

      filter[:subject_codes].split(",")
    end

    def provider_name
      return [] if filter[:query].blank?
